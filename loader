...
if not game.Workspace:FindFirstChild("Moon_Instance") then
	...
	local bedwars = { -- this is litterally 1/2 parts of the script that isnt mine :/
	...
	local Killaura = windowapi.CreateButton({
		...
		["Function"] = function(callback)
			if callback then -- credits to vape for being an example for me of how to add anims
				local animations = {
					Normal = {
						{CFrame = CFrame.new(1, -1, 2) * CFrame.Angles(math.rad(300), math.rad(55), math.rad(290)), Time = 0.3},
						{CFrame = CFrame.new(-1, 1, -2.2) * CFrame.Angles(math.rad(200), math.rad(60), math.rad(1)), Time = 0.3}
					}
				}
				local playinganimations = true
				...
				repeat
					...
					for _,v in pairs(game.Players:GetPlayers()) do
						if v then
							if v ~= lplr then
								nearestID = v
								if v.Team ~= lplr.Team and isalive(v) and (v.Character.HumanoidRootPart.Position - lplr.Character.HumanoidRootPart.Position).Magnitude < 19 then
									target = v
									local checkmode = false
									local sword = getSword()

									if not isalive(target) or (v.Character.HumanoidRootPart.Position - lplr.Character.HumanoidRootPart.Position).Magnitude > 19 then
										target = nil
									end
									local function hit()
										for _,v in pairs(animations.Normal) do
									...
									if sword ~= nil then
										...
										local function swing()
											checkmode = false
											local instancePART = Instance.new("Part",workspace)
											instancePART.Size = Vector3.new(7,12,7)
											instancePART.Name = "IAMAPART2"
											instancePART.Anchored = true
											instancePART.BrickColor = BrickColor.new("Royal purple")
											instancePART.Shape = Enum.PartType.Ball
											instancePART.CanCollide = false
											instancePART.Material = Enum.Material.Neon
											local g = Instance.new("Highlight",instancePART)
											g.OutlineTransparency = 1
											g.FillColor = Color3.fromRGB(144, 0, 142)
											for i = 1,5 do wait()
												instancePART.CFrame = v.Character.HumanoidRootPart.CFrame
											end
											instancePART.Name = "IAMAPART"
											for _,z in pairs(workspace:GetChildren()) do
												if z.Name == "IAMAPART" then
													instancePART:Destroy()
												end
											end
											task.wait(0.2)
											if not isalive(v) or (v.Character.HumanoidRootPart.Position - lplr.Character.HumanoidRootPart.Position).Magnitude > 19 then
												for _,z in pairs(workspace:GetChildren()) do
													if z.Name == "IAMAPART" or "IAMAPART2" then
														instancePART:Destroy()
													end
												end
											end
										...
										Client:Get(getremote(debug.getconstants((KnitClient.Controllers.SwordController).attackEntity))):SendToServer({
											["weapon"] = sword.tool,
									...
									pcall(function()
										if not isalive(v) or (v.Character.HumanoidRootPart.Position - lplr.Character.HumanoidRootPart.Position).Magnitude > 19 then
											...
											if cam.Viewmodel.RightHand.RightWrist.C0 ~= origC0 then
												anim = game:GetService("TweenService"):Create(cam.Viewmodel.RightHand.RightWrist, TweenInfo.new(0.1), {C0 = origC0})
												anim:Play()
											end
										else
	...
	local s7 = 6
	local s6 = lplr.Character.HumanoidRootPart.CFrame + lplr.Character.HumanoidRootPart.CFrame.LookVector * 50 + Vector3.new(0,10,0) -- magnitude slipper (basically ruins the anticheat)
	...
	local LongJump = windowapi.CreateButton({
		...
		["Function"] = function(callback)
			if callback then
				...
				local tween3 = tweenService:Create(game:GetService("Players")["LocalPlayer"].Character.HumanoidRootPart, tweenInfo3, {CFrame = s6})
				tween:Play()
				tween3:Play()
				task.wait(s2+0.1)
				tween:Play()
	...
	local Stealer = windowapi.CreateButton({ -- 100% credits to xylex
		["Name"] = "Stealer",
		["Tab"] = "Utility",
		["Function"] = function(callback)
			if callback then
